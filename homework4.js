...
function lnameValidation() {
    lname = document.getElementById('lname').value;
     ...
     if (lname == '') {
         ...
         if (!lname.match(lnamePattern)) {
             ...
             document.getElementById("lerror").innerHTML = "Last name must contain at least 1 character!";
             return false;
 ...
 function emailValidation() {
    ...
    if (email == '') {
        ...
        document.getElementById("emailerror").innerHTML = "Email address must be valid and in name@domain.tld format!";
        return false;
    } else {
...
function socialValidation() {
    ...
    let social = socialInput.value;
    if (social.trim() === '') {
        document.getElementById("socialerror").innerHTML = "Field must not be empty.";
        return false;
    }
    const cleaned = social.replace(/\D/g, '');
    const socialPattern = /^\d{9}$/;
    if (!socialPattern.test(cleaned)) {
        document.getElementById("socialerror").innerHTML = "Must match the following format: xxx-xx-xxxx";
        return false;
    }
    const formattedSocial = cleaned.substring(0, 3) + '-' + cleaned.substring(3, 5) + '-' + cleaned.substring(5);
    socialInput.value = formattedSocial;
    document.getElementById("socialerror").innerHTML = "";
    return true;
}
function phoneValidation(){
  ...
  zipInput.addEventListener("input", () => {
    ...
    if (isValidZip) {
      ...
      zipInput.setCustomValidity(
        "Please enter a valid zip code (5 digits i.e. 12345).");
      zipError.innerHTML =
        "Please enter a valid zip code (5 digits i.e. 12345).";
    }
  });
function userValidation(){
    ...
    if(user == ''){
        ...
        if(!user.match(userPattern)){
            ...
            error.innerHTML = "Username must not exceed 20 characters.";
            return false;
